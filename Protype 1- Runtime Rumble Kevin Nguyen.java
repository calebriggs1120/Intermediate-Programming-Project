...
class Game extends JPanel implements Runnable, KeyListener {
    ...
    public void update() {
        ...
        if (Math.random() < 0.01 && powerUps.size() < 3) { // 1% chance per frame, max 3 active
        ...
        // Update power-ups (gravity and falling)
        int groundY = getHeight() - getHeight() / 8;
        for (PowerUp powerUp : powerUps) {
            powerUp.update(groundY, getWidth());
        }
...
class Player {
    ...
    public void collectPowerUp(int type) {
        ...
        } else if (type == 2) { // Health recovery
            health = Math.min(100, health + 25); // Heal 25 HP (max 100)
        }
    }
}
...
class PowerUp {
    ...
    public void draw(Graphics g) {
        if (type == 0) { // Damage boost - red
            ...
            g.drawString("1.5X", xPosition + 3, yPosition + 22);
        } else if (type == 1) { // Speed boost - cyan
            g.setColor(Color.CYAN);
            g.fillOval(xPosition, yPosition, width, height);
            g.setColor(Color.BLACK);
            g.setFont(new Font("Arial", Font.BOLD, 14));
            g.drawString(">>", xPosition + 4, yPosition + 22);
        } else if (type == 2) { // Health boost - green
            g.setColor(Color.GREEN);
            g.fillOval(xPosition, yPosition, width, height);
            g.setColor(Color.WHITE);
            g.setFont(new Font("Arial", Font.BOLD, 14));
            g.drawString("+25", xPosition + 2, yPosition + 22);
        }
